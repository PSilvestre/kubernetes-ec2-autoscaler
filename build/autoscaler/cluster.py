...
class Cluster(object):
    ...
    def scale_loop(self):
        ...
        try:
            ...
            logger.info("Asgs found:")
            for asg in asgs:
                logger.info(asg.name)
            ...
            for p in running_or_pending_assigned_pods:
                pods_by_node.setdefault(p.node_name, []).append(p) #Creates map node -> [] pods running in it. Might have an empty key entry with all pending pods
